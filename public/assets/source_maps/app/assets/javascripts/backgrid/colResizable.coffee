(($) ->
  ...
  init = (table, options) ->
    ...
    if options.defaultSplits
      proportions = options.defaultSplits.split(";")
      $table.dS = _.map(proportions, (widthpercent) -> 
        (parseFloat(widthpercent)/100.0*$table.width())
      , $table)
      $table.dS.push($table.width())
      $table.dS = $table.dS.join(";")
    $table.gc = $table.prev()
  ...
  createGrips = (t) ->
    ...
    th = t.find '>thead>tr>th:visible,>thead>tr>td:visible'
    # but headers can also be included in different ways
    unless th.length
      selectors = [
        '>tbody>tr:first>th:visible'
        '>tr:first>th:visible'
        '>tbody>tr:first>td:visible'
        '>tr:first>td:visible'
      ]
    ...
    memento t, th, t.dS if t.dS
    memento t, th, S[t.id] if t.p and S and S[t.id]
  ...
  @param {Array} w - an array of the widths
  ###
